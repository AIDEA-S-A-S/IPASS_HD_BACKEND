...
const generateFile = (obj, description) => {
  ...
  Object.keys(obj).forEach(type => {
    ...
    if (includeDeprecatedFields || !field.isDeprecated) {
      ...
      query = `export const ${type} = /* GraphQL */\` \n ${description.toLowerCase()} ${type}${
...
//Merge all queries in one document
shellExec(`cd ${path.join(destDirPath, './queries')} && cat *.ts > allQueries.ts`)
  .then(console.log)
  .catch(console.log)

//Merge all mutations in one document
shellExec(`cd ${path.join(destDirPath, './mutations')} && cat *.ts > 0allMutations.ts`)
  .then(console.log)
  .catch(console.log)

shellExec(`cd ${path.join(destDirPath, './subscriptions')} && cat *.ts > allSubscriptions.ts`)
  .then(console.log)
  .catch(console.log)
